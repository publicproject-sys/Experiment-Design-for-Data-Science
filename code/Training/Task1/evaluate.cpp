...
Res testSequential(vector<vector<string> > &sents, 
  ...
  for (unsigned int i=0; i<sents.size(); i++) { // per sentence
    ...
    vector<pair<unsigned int,unsigned int> > pred, tru;
    int l1=-1, l2=-1;

    if (labels[i].size() != labelsPredicted.size())
      cout << labels[i].size() << " " << labelsPredicted.size() << endl;
    for (unsigned int j=0; j<labels[i].size(); j++) { // per token in a sentence
      t = labels[i][j];
      y = labelsPredicted[j];
    ...
    for (unsigned int a=0; a<tru.size(); a++) {
      pair<unsigned int,unsigned int> truSpan = tru[a];
      nExprTrue++;
      for (unsigned int b=0; b<pred.size(); b++) {
        pair<unsigned int,unsigned int> predSpan = pred[b];

        unsigned int lmax, rmin;
